...
def test_root_hostname(testapp):
    ...
    resp.mustcontain("easy_install -i http://systemexit.de/pypi/web/simple/ PACKAGE")
...
def test_fallback(root, _app, testapp):
    ...
    resp = testapp.get("/pypi/web/simple/pypiserver/", status=302)
    assert resp.headers["Location"] == "https://pypi.org/pypi/web/simple/pypiserver/"


def test_no_fallback(root, _app, testapp):
    _app.config.redirect_to_fallback = False
    testapp.get("/pypi/web/simple/pypiserver/", status=404)
...
def test_simple_redirect(testapp):
    resp = testapp.get("/pypi/web/simple")
    assert resp.status_code >= 300
    assert resp.status_code < 400
    assert resp.location.endswith('/pypi/web/simple/')
...
def test_simple_list_no_dotdir2(root, testapp):
    root.mkdir(".subdir").join("foo-1.0.zip").write("secret")
    resp = testapp.get("/pypi/web/simple/foo/")
    assert resp.html("a") == []


def test_simple_name_redirect(testapp):
    resp = testapp.get("/pypi/web/simple/foobar")
    assert resp.status_code >= 300
    assert resp.status_code < 400
    assert resp.location.endswith('/pypi/web/simple/foobar/')
...
def test_simple_normalized_name_redirect(testapp, package, normalized):
    resp = testapp.get("/pypi/web/simple/{0}/".format(package))
    assert resp.status_code >= 300
    assert resp.status_code < 400
    assert resp.location.endswith('/pypi/web/simple/{0}/'.format(normalized))
...
def test_nonroot_root(testpriv):
    ...
    resp.mustcontain("easy_install -i http://nonroot/priv/pypi/web/simple/ PACKAGE")


def test_nonroot_simple_index(root, testpriv):
    root.join("foobar-1.0.zip").write("")
    resp = testpriv.get("/priv/pypi/web/simple/foobar/")
    links = resp.html("a")
    assert len(links) == 1
...
def test_root_no_relative_paths(testpriv):
    ...
    assert hrefs == ['/priv/packages/', '/priv/pypi/web/simple/',
...
def test_simple_index_egg_and_tarball(root, testapp):
    ...
    resp = testapp.get("/pypi/web/simple/foo-bar/")
    assert len(resp.html("a")) == 2
